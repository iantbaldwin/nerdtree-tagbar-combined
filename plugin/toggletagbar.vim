function s:OpenNERDTreeAndTagbar()
    ...
    if !s:tagbar_open || !s:nerdtree_open
        if s:nerdtree_open
            NERDTreeClose
...
function s:CloseNERDTreeAndTagbar()
    if exists('g:tagbar_left')
        let s:tagbar_left_user = g:tagbar_left
    else
        let s:tagbar_left_user = 0
...
command! -nargs=0 OpenNERDTreeAndTagbar :call s:OpenNERDTreeAndTagbar()
command! -nargs=0 CloseNERDTreeAndTagbar :call s:CloseNERDTreeAndTagbar()
